...
func GenerateTokens(user_id int) (map[string]string, error) {
	tokenString, err := GenerateToken(0)
	if err != nil {
		return nil, err
	}

	refreshTokenString, err := GenerateRefreshToken(0)
	if err != nil {
		return nil, err
	}
...
func GetToken(token string) (*jwt.Token, error) {
	tokenOut, err := jwt.Parse(token, func(token *jwt.Token) (interface{}, error) {
		if _, ok := token.Method.(*jwt.SigningMethodHMAC); !ok {
			return nil, fmt.Errorf("unexpected signing method: %v", token.Header["alg"])
		}
		return []byte(os.Getenv("API_SECRET")), nil
